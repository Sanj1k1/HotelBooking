...
class BookingSerializer(ModelSerializer):
    """Serializer for Booking model with validation and calculated fields."""
    ...
    class Meta:
        model = Booking
        fields = [
            'id', 'user', 'room', 'room_number', 'hotel_name', 
            'check_in', 'check_out', 'total_price', 'status', 'payment'
        ]
        read_only_fields = [
            'id', 'user', 'room_number', 'hotel_name', 'total_price', 'payment'
        ]
    ...
    def validate(self, attrs):
        ...
        if check_in < timezone.now().date():
            raise ValidationError({
                "check_in": "The arrival date cannot be in the past."
            })
        ...
        if room:
            ...
            if overlapping:
                raise ValidationError({
                    "room": "The room is already booked for the specified dates"
                })
        
        return attrs
    
    def create(self, validated_data):
        """Create a booking with the authenticated user as the booker."""
